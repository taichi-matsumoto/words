...
so that構文: この例文も「so that構文」とみてよい。thatを補完すると、It's so unimportant that it's hardly worth mentioning.であるが、hardlyはnotにしてしまって、It's so unimportant that it's not worth mentioning.とすればわかり良い
